...
This repo includes a **core library** (`risk_pipeline/`) and **paper-grade reproduction scripts** (`experiments/`).

### Quickstart (simulator-only end-to-end)

```bash
pip install -r requirements.txt
python experiments/end_to_end_sim_demo.py --out artifacts
```

### Train QRU forecaster (sim)

```bash
pip install -r requirements.txt
pip install pyyaml
python experiments/qru_train.py --config configs/paper_qru_72params.yaml --csv path/to/site_timeseries.csv --out artifacts
```

### QAOA inference via qBraid (IonQ simulator)
...
```bash
export QBRAID_API_KEY=...  # if required
python experiments/run_qaoa_inference_qbraid_ionq.py --artifacts artifacts/demo_qaoa_artifacts.npz --device-id ionq_simulator --shots 10000
```

### IBM QPU scripts

The scripts under `experiments/` do **not** ship any credentials. Provide them via environment variables (see `configs/paper_qpu_ibm.yaml`).

## Reproducibility map

See `docs/reproducibility.md` for a paper ↔ code mapping.
